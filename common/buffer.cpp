...
int32_t BufferManager::Initialize(char *b, int32_t nb)
{
...
void BufferManager::Write4Bytes(int32_t value)
{
    rs_assert(Require(4));
    char *pp = (char *)&value;
    rs_verbose("write4Bytes:%c, %c, %c, %c", pp[0], pp[1], pp[2], pp[3]);
    *ptr_++ = pp[3];
    *ptr_++ = pp[2];
    *ptr_++ = pp[1];
    *ptr_++ = pp[0];
...
FastBuffer::FastBuffer() : merge_read_(false),
                           mr_handler_(nullptr)
{
...
void FastBuffer::SetBuffer(int32_t buffer_size)
{
...
char *FastBuffer::ReadSlice(int32_t size)
{
    rs_assert(size >= 0);
    rs_assert(Size() >= size);
...
int32_t FastBuffer::Grow(IBufferReader *r, int required_size)
{
    rs_assert(required_size > 0);
    ...
    int32_t free_space = (int32_t)(buf_ + capacity_ - end_);
    int32_t used_space = (int32_t)(end_ - start_);
    ...
    if (free_space < required_size - used_space)
    {
        ...
        free_space = (int32_t)(buf_ + capacity_ - end_);

        // avoid buffer overflow, which cause function never return
        if (free_space < required_size - used_space)
        {
            ret = ERROR_READER_BUFFER_OVERFLOW;
            rs_error("buffer overflow, required=%d, max=%d, left =%d, ret=%d", required_size, capacity_, free_space, ret);
            return ret;
        }
    }
    ...
    while (end_ - start_ < required_size)
    {
        ...
        if (merge_read_ && mr_handler_)
        {
            mr_handler_->OnRead(nread);
        }

        rs_assert(int32_t(nread) > 0);
        end_ += nread;
        free_space -= nread;
    }
    return ret;
...
void FastBuffer::Skip(int32_t size)
{
    rs_assert(Size() >= size);
    rs_assert(start_ + size >= buf_);
    start_ += size;
