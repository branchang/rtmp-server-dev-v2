...
static int amf0_read_utf8(BufferManager *manager, std::string &value)
{
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 read empty string, ret=%d", ret);
        return ret;
    }
...
{
    // 1 bytes marker + 8 bytes number length
...
int AMF0EcmaArray::Read(BufferManager *manager)
{
    ...
    rs_verbose("amf0 read ecma array count success,count=%d", count);
...
int AMF0EcmaArray::Write(BufferManager *manager)
{
    ...
    for (int i = 0;i<properties_->Count(); i++)
    {
        ...
        {
            rs_error("amf0 write ecma arrya property name failed, ret=%d", ret);
            return ret;
        }
    ...
    rs_verbose("amf0 wirte ecma array success");
    return ret;
}
...
int AMF0Date::Read(BufferManager *manager)
{
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 read date check marker failed, marker=%#x, required=%#x", marker, RTMP_AMF0_DATE);
        return ret;
    }
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 read tiem_zone failed, ret=%d", ret);
        return ret;
    }

    time_zone_ = manager->Read2Bytes();

    rs_verbose("amf0 read time_zone  success,timezone=%d", time_zone_);
...
{
    // 1bytes marker + 8bytes date_value + 2 bytes time_zone
...
static int amf0_read_utf8(BufferManager *manager, std::string &value)
{
    ...
    rs_verbose("amf0 read string length success, lent=%d", len);
...
int AMF0ReadString(BufferManager *manager, std::string &value)
{
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 check string marker check failed, marker=%#x, required=%#x, ret=%d", marker, RTMP_AMF0_STRING, ret);
        return ret;
    }
...
int AMF0ReadNumber(BufferManager *manager, double &value)
{
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 read number marker check failed, ret=%d", ret);
        return ret;
    }
    ...
    rs_verbose("amf0 read number data success,value=%.1f", value);
...
int AMF0WriteNumber(BufferManager *manager, double value)
{
    ...
    rs_verbose("amf0 wirte number data success, data=%.1f", value);
    rs_verbose("amf0 write number success");
    return ret;
}

static int amf0_wirte_utf8(BufferManager *manager, const std::string &value)
{
    int ret = ERROR_SUCCESS;
    ...
    manager->Write2Bytes(value.length());
    rs_verbose("amf0 write string length succes,len=%d", (int)value.length());
    ...
    rs_verbose("amf0 write string ddata success");
...
int AMF0WriteString(BufferManager *manager, const std::string &value)
{
    ...
    return amf0_wirte_utf8(manager, value);
...
int AMF0ReadBoolean(BufferManager *manager, bool &value)
{
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 read boolean marker failed, ret=%d", ret);
        return ret;
    }
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 read boolean check marker failed,marker=%#x,required=%#x,ret=%d", marker, RTMP_AMF0_BOOLEAN, ret);
        return ret;
    }
...
int AMF0WriteBoolean(BufferManager *manager, bool value)
{
    ...
    rs_verbose("amf0 write bool value marker success, value=%d", value);
    rs_verbose("amf0 write undefined success");
...
int AMF0ReadNull(BufferManager *manager)
{
    ...
    {
        ret = ERROR_RTMP_AMF0_DECODE;
        rs_error("amf0 read null check marker failed,marker=%#x,required=%#x,ret=%d", marker, RTMP_AMF0_NULL, ret);
        return ret;
    }
